...
local HumpCam = require("lib.hump_camera")
local Vector2 = require("lib.point")
...
function Camera:checkInputs(dt)
  ...
  local centerX = w / 2
  local centerY = h / 2
  if self.camera.x - centerX < 0 then self.camera.x = centerX end
  if self.camera.x + centerX > tilemapW then self.camera.x = tilemapW - centerX end
  if self.camera.y + centerY < 0 then self.camera.y = -centerY end
  if self.camera.y > centerY then self.camera.y = centerY end
end
